...
Second, replace the leftmost group of consecutive, zero-value
hextets with double colons, producing the shorter

	fd00:4700:10::6814:d103.

If your address has more than one group of consecutive zero-value hextets, you
can remove only the leftmost group.
*/
func ShortenIPv6Address(ipv6 string) string {
	var segments []string

	for _, segment := range strings.Split(ipv6, ":") {
		stripped := stripLeadingZerosFromString(segment)
	...
	leftSegments := strings.Join(segments[:startIndex], ":")
	rightSegments := strings.Join(segments[stopIndex+1:], ":")
...
func stripLeadingZerosFromString(str string) string {
	result := str
...
func validateIPv6Address(ipv6 string) (string, error) {
	...
	// strip zone identifier %eth0 on IPv6 address
